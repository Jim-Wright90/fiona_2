...
ggplot(fiona, aes(date, total_diaper)) +
  geom_line(lwd = 1.6,
            color = "gray40") +
  geom_point(size = 2, 
             color = "blue") +
...
fiona_table <- fiona_table %>% 
  rename("Total Diapers" = "mean(total_diaper)",
         "Wet Diapers" = "mean(total_wet)",
         "Dirty Diapers" = "mean(total_dirty)",
         "Both Diapers" = "mean(total_both)",
         "Feedings" = "mean(feedings)")
...
reactable(fiona_table, columns = list(
  "Total Diapers" = colDef(format = colFormat(digits = 2)),
  "Wet Diapers" = colDef(format = colFormat(digits = 2)),
  "Dirty Diapers" = colDef(format = colFormat(digits = 2)),
  "Both Diapers" = colDef(format = colFormat(digits = 2)),
  "Feedings" = colDef(format = colFormat(digits = 2))
))
